...
public class ActivityFamily extends SherlockFragmentActivity implements OnClickListener{
	TabHost mTabHost;
	...
	private void setupTabs()
	{
		...
		Bundle family = new Bundle();
		family.putString("type", "family");
		family.putInt("dremer_id", mDremerId);

		mTabsAdapter.addTab(mTabHost.newTabSpec("friends").setIndicator("Family"),
				FragmentDremers.class, family);
		if (mDremerId == Integer.parseInt(mPrefs.getUserId())) {
			Bundle requests = new Bundle(); requests.putString("type", "familyship_request");
			requests.putInt("dremer_id", mDremerId);

			mTabsAdapter.addTab(mTabHost.newTabSpec("requests").setIndicator("Requests"),
					FragmentDremers.class, requests);
		}
	}
